...
class NotifiactionParserTest {
    ...
    fun testKukuruza() {
        val check = parseNotification("Кукуруза",
                """-215.00 RUR
                  |CKASSA, YAROSLAVL. Остаток  0 000.00 RUR. Карта *0000"""
                        .trimMargin())
        assertNotNull(check)
    ...
    fun testAlfa() {
        val check = parseNotification("Уведомление",
                "**0000 Pokupka Uspeshno Summa: 309,51 RUR Ostatok: 0 000,00 RUR RU/Novosibirsk/YARMARKA DOBRYANKA 04.07.2019 15:22:04")
        assertNotNull(check)
    ...
    fun testAlfa2() {
        val check = parseNotification("Уведомление",
                "**0000 Pokupka Uspeshno Summa: 1 309,51 RUR Ostatok: 0 000,00 RUR RU/Novosibirsk/YARMARKA DOBRYANKA 04.07.2019 15:22:04")
        assertNotNull(check)
        assertEquals("RU/Novosibirsk/YARMARKA DOBRYANKA", check!!.place)
    ...
    fun testAlfa3() {
        val check = parseNotification("Уведомление",
                "Списание со счета 000*00000 на сумму 1,770.00 RUR; 06.07.2019 18:27:09.")
        assertNotNull(check)
    ...
    fun testAlfa5() {
        val check = parseNotification("Уведомление",
                "**0000; Покупка совершена успешно. Сумма: 437,45 RUB; Остаток: 0 000,00 RUB; Птицефабрика «Октябрьская»; 26 февраля 2020 08:56")
        assertNotNull(check)
